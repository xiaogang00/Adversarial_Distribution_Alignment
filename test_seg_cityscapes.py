...
def main():
    ...
    save_folder = 'checkpoints/cityscapes_model_pspnet/results'
    save_folder_list = [save_folder]

    save_path = 'checkpoints/cityscapes_model_pspnet/latest_net_G.pth'
    save_path_list = [save_path]

    test_transform = transform.Compose([transform.ToTensor()])
    split = 'val'
    data_root = '/mnt/backup/project/hszhao/dataset/cityscapes'
    test_list = '/mnt/backup/project/hszhao/dataset/cityscapes/list/fine_val.txt'
    ...
    colors_path = '/mnt/proj3/xgxu/semseg_mp/dataset/cityscapes/cityscapes_colors.txt'
    names_path = '/mnt/proj3/xgxu/semseg_mp/dataset/cityscapes/cityscapes_names.txt'
    ...
    model_path = 'model_seg_NoD/cityscapes/pspnet/train_epoch_100.pth'
    checkpoint = torch.load(model_path)
    model.load_state_dict(checkpoint['state_dict'], strict=False)
    print('loading successfully')

    model_black = DeepLabV3(layers=50, classes=19, zoom_factor=8, pretrained=False)
    model_black = torch.nn.DataParallel(model_black).cuda()
    cudnn.benchmark = True
    model_path = 'model_seg_NoD/cityscapes/deeplab/train_epoch_100.pth'
    checkpoint = torch.load(model_path)
